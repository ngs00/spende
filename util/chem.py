...
def get_fvec(elem_dict, elem_attrs):
...
#
#     for e in elem_dict.keys():
#         list_atom_feats.append(elem_attrs[atom_nums[e] - 1, :])
#     atom_feats = torch.vstack(list_atom_feats)
#
#     form_vec = torch.hstack([torch.mean(atom_feats, dim=0),
#                              torch.min(atom_feats, dim=0)[0],
#                              torch.max(atom_feats, dim=0)[0]])
#
#     return form_vec


def is_valid_formula(elem_dict):
    valid = True

    if len(elem_dict.keys()) == 0:
        return False

    for e in elem_dict.keys():
        if e not in atom_nums.keys():
            valid = False
            break

        if re.search('[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?$', elem_dict[e]) is None:
            valid = False
            break
